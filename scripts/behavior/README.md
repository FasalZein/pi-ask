...
Run from the repository root, outside `pnpm test`. Each case has a fresh Pi process and session. The six original cases plus `interview` use RPC with the in-process auto-answer bridge. The bridge uses `pi-ask:started` and `pi-ask:submit`, not terminal keys or RPC dialog responses. `subagent` runs `pi -p --mode json`, with `--tools read,ask_user` and `--no-context-files`, matching a background child's print mode and restricted tool list. Both paths use only this worktree's `src/index.ts` via `--no-extensions -e`; print mode needs no answer bridge. Full mode can call `ask_user` but cannot open its UI; compact mode disables it in print mode.
