...
The default `ask_user` prompt mode is `compact`, with shorter tool text. Set `PI_ASK_PROMPT_MODE=full` in pi's environment before starting pi to use the upstream v1.2.0 tool text. The extension reads the variable once at load; restart or reload pi after changing it. Unset, empty, and `compact` select compact mode. Any other value selects compact mode and logs one warning. Compact mode puts follow-up rules in tool text instead of results, asks for option labels only, derives unique machine values from labels, and limits each ask to four questions. Older calls with valid explicit values still work. Full mode still requires explicit values. Compact mode sends conditional configuration guidance when the user mentions pi-ask settings or keys.
