...
The default `ask_user` prompt mode is `full`, with the upstream v1.2.0 tool text. Set `PI_ASK_PROMPT_MODE=compact` in pi's environment before starting pi to use shorter tool text. The extension reads the variable once at load; restart or reload pi after changing it. Unset, empty, and `full` select full mode. Any other value selects full mode and logs one warning. Compact mode puts follow-up rules in tool text instead of results, fills missing option values from labels with unique slugs, and limits each ask to four questions. Full mode still requires explicit values. Compact mode sends conditional configuration guidance when the user mentions pi-ask settings or keys.
