...
The `PI_ASK_PROMPT_MODE` environment variable selects the model-facing tool text at extension load. Compact is the default: leave it unset, set it to empty, or use `compact` for the shorter tool description, two rule guidelines, instructional option `label` and `recommended` parameter descriptions, up to four questions per ask, and unique label-derived machine values. The `questions` description matches full mode. Compact options expose only the label as an identifier; valid explicit values from older calls still work. Full mode still requires explicit option values. Use `full` for the upstream v1.2.0 text. Unknown values fall back to compact mode with one warning. Restart or reload pi to apply a change. This variable is not a key in `pi-ask.json`. Compact mode places follow-up rules in guideline 2, not in the `questions` description or results. When a choice is still needed after an answer or note, use another structured call instead of plain-text choices; bundle 2-3 related unresolved decisions when prior answers narrow the branch and when possible, and ask one at a time only when the next question materially depends on the previous answer. It sends conditional configuration guidance when the user mentions pi-ask settings or keys.
