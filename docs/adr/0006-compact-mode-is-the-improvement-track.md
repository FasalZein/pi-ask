...
Compact becomes the default after two checks. First, the base-model gate (ADR 0003) passes on the current compact text. Second, a report run on `anthropic/claude-opus-5-5` and `openai-codex/gpt-6-sol` shows no behavior regression against full mode. ADR 0007 records that switch and replaces 0005's default.
